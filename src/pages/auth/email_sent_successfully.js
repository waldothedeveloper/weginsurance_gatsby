import { Link } from "gatsby";
import PropTypes from "prop-types";
import React from "react";
export default function EmailSentSuccessfully({ location }) {
  const { email } = location.state ?? { email: `email not provided` };

  return (
    <div className="h-screen">
      <div className="min-h-full bg-white px-4 py-16 sm:px-6 sm:py-24 md:grid md:place-items-center lg:px-8">
        <div className="mx-auto max-w-max">
          <main className="sm:flex">
            <p
              className={
                email === `email not provided`
                  ? `text-4xl font-extrabold text-red-600 sm:text-5xl`
                  : `text-4xl font-extrabold text-sky-600 sm:text-5xl`
              }
            >
              {email === `email not provided` ? 500 : 200}
            </p>
            <div className="sm:ml-6">
              <div className="sm:border-l sm:border-gray-200 sm:pl-6">
                <h1
                  className={
                    email === `email not provided`
                      ? `text-4xl font-extrabold tracking-tight text-red-600 sm:text-5xl`
                      : `text-4xl font-extrabold tracking-tight text-gray-900 sm:text-5xl`
                  }
                >
                  {email === `email not provided`
                    ? `Error:`
                    : `Correo enviado con éxito!`}
                </h1>
                {email === `email not provided` ? (
                  <p className="mt-1 text-base font-semibold text-gray-500">
                    Lo sentimos, hubo un error al enviar el correo.
                  </p>
                ) : (
                  <p className="mt-1 text-base font-semibold text-gray-500">
                    Te enviamos un correo a{` `}
                    <span className="font-bold text-sky-800">{email}</span>
                    {` `}con un enlace para recuperar tu contraseña
                  </p>
                )}
              </div>
              <div className="mt-10 flex space-x-3 sm:border-l sm:border-transparent sm:pl-6">
                <Link
                  data-testid="back-to-home"
                  to="/"
                  className="inline-flex items-center rounded-md border border-transparent bg-sky-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2"
                >
                  Regresar al inicio
                </Link>
                {email === `email not provided` ? null : (
                  <a
                    data-testid="gmail-link"
                    href={`https://mail.google.com/mail/u/0/#inbox`}
                    target="_blank"
                    rel="noreferrer noopener"
                    className="inline-flex items-center rounded-md border border-transparent bg-sky-100 px-4 py-2 text-sm font-medium text-sky-700 hover:bg-sky-200 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2"
                  >
                    Llévame a mi Gmail
                  </a>
                )}
              </div>
            </div>
          </main>
        </div>
      </div>
    </div>
  );
}

EmailSentSuccessfully.propTypes = {
  location: PropTypes.object,
};
