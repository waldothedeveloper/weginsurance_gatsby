...
export const useDeliveryReceipt = (
  ...
  const messageSids = useMemo(() => fromApp.map((m) => m.sms_sid), [fromApp]);
  ...
  const { data: deliveryReceipt } = useSWR(
    () => {
      ...
      if (sms_sid && messageSids && messageSids.length > 0)
        return `/api/sms_delivery_update`;
      if (sms_sid && messageSids && messageSids.length > 0 && newMessageSent)
        return `/api/sms_delivery_update`;
      return null;
    },
    () => checkDeliveryReceipt(sms_sid, messageSids),
    {
